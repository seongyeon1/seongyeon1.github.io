...
| 발행완료 | 2026-09-05 | [논문 리뷰] SENTINEL-RL — SOC agent의 graph reasoning을 외부 엔진에 맡기기 | paper-review | AI 평가/운영 | | `content/posts/2026-09-05-sentinel-rl-soc-agent-graph-reasoning-offload.mdx` — 자동 발행 완료, arXiv 2609.04159 — SOC agent의 topology reasoning offload, graph query tool boundary, alert triage evidence trace 설계 |
| 발행완료 | 2026-09-19 | [논문 리뷰] OverclaimBench — coding agent가 “끝냈다”고 과장하는 순간 측정하기 | paper-review | AI 평가/운영 | 21 | `content/posts/2026-09-19-overclaimbench-coding-agent-completion-reporting.mdx` — 자동 발행 완료, arXiv 2609.20812 — final response와 context/trace의 모순으로 overclaim을 정의하고 planted defect·transcript coverage로 장시간 coding agent의 완료 보고 신뢰성을 평가 |
| 발행완료 | 2026-09-24 | [논문 리뷰] CliffCompaction — long-horizon coding agent의 context compaction 비용 줄이기 | paper-review | 개발자 AI 워크플로 | | `content/posts/2026-09-24-cliffcompaction-cost-efficient-context-compaction-coding-agents.mdx` — 자동 발행 완료, arXiv 2609.26779 — bounded context에서 autocompaction으로 Terminal-Bench/KernelBench 장기 coding agent의 rollout 비용을 줄이고 test-time scaling 효율을 높이는 방식 분석 |
| 발행완료 | 2026-09-26 | [논문 리뷰] Trace Tampering — LLM agent가 자기 실행 로그를 지울 수 있다면 | paper-review | AI 평가/운영 | | `content/posts/2026-09-26-trace-tampering-agent-trace-integrity.mdx` — 자동 발행 완료, arXiv 2609.30266 — local coding agent가 자기 trace를 삭제할 수 있는 trace integrity 실패를 independent interception, append-only audit log, host compromise 가정으로 분석 |
| 아이디어 | 미정 | [논문 리뷰] EvasionBench — 평범한 작업 압력만으로 monitor 회피가 나타나는가 | paper-review | AI 평가/운영 | | arXiv 2609.30217 — task completion과 runtime monitor가 충돌할 때 나타나는 instrumental monitor evasion을 tool policy, approval gate, monitor-aware harness 평가로 정리 |
| 아이디어 | 미정 | [논문 리뷰] HEXIS — agent skill을 extended finite state machine으로 컴파일하기 | paper-review | Agent/MCP 실전 | | arXiv 2609.30123 — markdown skill을 EFSM control flow로 컴파일해 prescribed step 누락을 줄이고 progress/state를 명시하는 skill runtime 패턴 분석 |
| 아이디어 | 미정 | [논문 리뷰] Jev-Mobile — mobile GUI agent에서 VLM planning과 typed executor 분리하기 | paper-review | 개발자 AI 워크플로 | | arXiv 2609.30186 — low-frequency VLM planning과 accessibility tree 기반 high-frequency typed execution으로 mobile GUI agent latency와 serving cost를 줄이는 구조 분석 |
| 아이디어 | 미정 | AtomicMemory — framework adapter와 MCP를 갖춘 portable semantic memory | tutorial | Agent/MCP 실전 | | GitHub repo 기반 — core engine, TypeScript SDK, framework adapters, MCP server, CLI를 갖춘 portable semantic memory substrate와 correction/deletion policy 소개 |
| 아이디어 | 미정 | [논문 리뷰] AEWM — tool response를 예측하지 말고 agent state를 편집하기 | paper-review | 개발자 AI 워크플로 | | arXiv 2609.28416 — tool response 예측보다 reasoning/action이 future task progress를 어떻게 바꾸는지 모델링하고 outdated plan·unsupported assumption을 편집하는 agent world model 분석 |
| 아이디어 | 미정 | [논문 리뷰] PASTABench — multi-step agent safety를 proactive intervention으로 평가하기 | paper-review | AI 평가/운영 | | arXiv 2609.28197 — 1,139개 multi-turn trajectory, risk taxonomy, Optimal Intervention Point로 agent safety를 사후 판정이 아니라 개입 타이밍 문제로 정리 |
| 아이디어 | 미정 | [논문 리뷰] SkillGym — human-written agent skill을 실행·검증 가능한 학습 환경으로 바꾸기 | paper-review | 개발자 AI 워크플로 | | arXiv 2609.27717 — skill-to-task pipeline, code-based checker, contrastive execution으로 prompt skill library를 verifiable training environment로 바꾸는 agent 학습 데이터 flywheel 분석 |
| 아이디어 | 미정 | [논문 리뷰] ProCredit — agentic RL에서 최종 성공 대신 진행 기여도를 보상하기 | paper-review | AI 평가/운영 | | arXiv 2609.27532 — long-horizon tool-use RL에서 outcome reward 대신 task를 전진시킨 turn에 credit을 주는 progress reward 설계와 verifier signal 활용 |
| 아이디어 | 미정 | Koog — JVM/Kotlin 생태계의 fault-tolerant production agent framework 보기 | tutorial | Agent/MCP 실전 | | GitHub repo 기반 — JetBrains Koog의 Kotlin API, cross-platform runtime, typed tool contract, fault-tolerant agent workflow를 JVM 백엔드 도입 체크리스트로 소개 |
| 아이디어 | 미정 | [논문 리뷰] A2M — MCP 생태계의 semantic tool hijacking 공격 | paper-review | AI 평가/운영 | | arXiv 2609.26761 — tool metadata와 반환값을 최적화해 malicious tool invocation을 유도하는 MCP semantic supply-chain 공격, registry trust와 approval/audit gate 설계 |
| 아이디어 | 미정 | [논문 리뷰] SWE-Serve — production inference serving 작업으로 coding agent 평가하기 | paper-review | AI 추론 최적화 | | arXiv 2609.26777 — model support, runtime execution, public API 변경이 얽힌 production inference engineering task로 serving stack coding agent 평가 설계 |
| 아이디어 | 미정 | [논문 리뷰] Agensh — 중앙 orchestrator 없이 1,024-agent 조직 지능 확장하기 | paper-review | Agent/MCP 실전 | | arXiv 2609.26781 — central orchestrator 병목을 줄이는 self-organized multi-agent loop, asynchronous task claiming, verification/merge protocol 분석 |
| 아이디어 | 미정 | Engram — coding agent memory를 agent-agnostic Go binary로 분리하기 | tutorial | 개발자 AI 워크플로 | | GitHub repo 기반 — SQLite+FTS5, MCP server, HTTP API, CLI/TUI로 Claude Code/Codex/Cursor 간 persistent memory를 공유하는 local subsystem 설계 |
| 아이디어 | 미정 | [논문 리뷰] Harness-Zero — agent harness를 모델 내부 능력으로 distill하기 | paper-review | Agent/MCP 실전 | | arXiv 2609.24974 — agent-as-harness distillation, deployment-time harness dependency, tool-use policy transfer를 agent runtime 단순화와 harness 최적화 관점으로 분석 |
| 아이디어 | 미정 | [논문 리뷰] DolphinBench — agent memory의 Pareto frontier를 측정하기 | paper-review | 개발자 AI 워크플로 | | arXiv 2609.24971 — long-term memory/context recall을 accuracy·cost·privacy frontier로 보고 implicit recall, memory budget, context packing 선택 기준 정리 |
| 아이디어 | 미정 | [논문 리뷰] Re:CAP — production RAG의 retrieval coverage를 감사하기 | paper-review | AI 평가/운영 | | arXiv 2609.24122 — non-stationary corpus와 re-indexing drift 속에서 retrieval coverage audit를 RAG observability와 agentic evidence gate로 연결 |
| 아이디어 | 미정 | judgeval — continuous-improvement stack으로 agent eval 운영하기 | tutorial | AI 평가/운영 | | GitHub repo 기반 — eval dataset, monitoring signal, failure clustering, CI/production feedback loop를 production agent 품질 관리 방식으로 소개 |
| 아이디어 | 미정 | [논문 리뷰] APort Vault — tool-using agent의 결제 승인 공격을 benchmark로 만들기 | paper-review | AI 평가/운영 | | arXiv 2609.22076 — Open Agent Passport 기반 결제 agent 공격 replay, authorization policy, approval boundary, audit log를 monetary action agent safety 관점으로 분석 |
| 아이디어 | 미정 | [논문 리뷰] CodeMidas — 코드 자체에서 agentic coding RL environment 확장하기 | paper-review | 개발자 AI 워크플로 | | arXiv 2609.22068 — open-source codebase를 executable task와 verifier로 바꿔 coding agent RL 데이터 flywheel을 만드는 방법과 rollout/검증 비용을 정리 |
| 아이디어 | 미정 | [논문 리뷰] Memory Decision Controller — LLM agent가 검색된 기억을 언제 믿어야 하나 | paper-review | 개발자 AI 워크플로 | | arXiv 2609.22043 — confidence·consistency·context 신호를 분리해 stale/contradictory memory 사용 여부를 결정하는 trust controller를 agent memory QA로 분석 |
| 아이디어 | 미정 | [논문 리뷰] AutoViewMem — 장기 대화 메모리를 self-configuring view로 구성하기 | paper-review | 개발자 AI 워크플로 | | arXiv 2609.21940 — 고정 schema 대신 orthogonal memory view를 자동 구성해 personalization consistency, retrieval cost, privacy trade-off를 조정하는 방식 분석 |
| 아이디어 | 미정 | Skybridge — MCP Apps와 ChatGPT Apps를 잇는 type-safe full-stack framework | tutorial | Agent/MCP 실전 | | GitHub repo 기반 — TypeScript contract, React-powered UI, MCP/App bridge, auth/runtime 배포 경계를 agent app framework 선택 기준으로 소개 |
| 아이디어 | 미정 | [논문 리뷰] Workspace Models — 로봇 agent memory를 saliency-supervised state로 줄이기 | paper-review | 개발자 AI 워크플로 | | arXiv 2609.20820 — 전체 trajectory 대신 salient workspace state를 유지하는 lightweight robotic memory를 agent state abstraction·history compression 관점으로 분석 |
| 발행완료 | 2026-09-22 | [논문 리뷰] Prediction-Powered Smoothing — agent 평가를 적은 라벨로 안정화하기 | paper-review | AI 평가/운영 | | `content/posts/2026-09-22-prediction-powered-smoothing-agent-eval-label-budget.mdx` — arXiv 2609.20758 — disaggregated AI evaluation에서 작은 라벨 예산과 예측 모델을 결합해 slice별 성능 추정과 drift validation을 안정화하는 통계 계층 정리 |
| 아이디어 | 미정 | [논문 리뷰] SAFARI — 안전 중요 워크플로에서 LLM agent 위험 분석 평가하기 | paper-review | AI 평가/운영 | | arXiv 2609.20584 — functional-safety hazard analysis benchmark를 통해 domain ontology, risk rubric, evidence trace, human reviewer handoff를 agent QA로 해석 |
| 아이디어 | 미정 | Deuz-SDK — durable execution과 long-term memory를 묶은 TypeScript agent framework | tutorial | Agent/MCP 실전 | | GitHub repo 기반 — durable workflow, hybrid RAG, MCP tool calling, HITL approval을 production agent framework 선택 체크리스트로 소개 |
| 아이디어 | 미정 | [논문 리뷰] Chronicle — LLM agent 실패를 cut-point replay로 회귀 테스트하기 | paper-review | AI 평가/운영 | | arXiv 2609.20625 — 비결정적 모델/tool state/긴 trajectory가 섞인 agent 실패를 cut-point snapshot과 replay로 CI 회귀 테스트하는 방법 분석 |
| 아이디어 | 미정 | [논문 리뷰] RAFT — troubleshooting agent를 위한 stateful RAG 프레임워크 | paper-review | Agent/MCP 실전 | | arXiv 2609.20754 — 고객지원·운영 troubleshooting agent에서 historical case retrieval과 diagnostic state 전이를 결합하는 stateful RAG 설계 |
| 아이디어 | 미정 | [논문 리뷰] SoL-Pi — auto-research agent harness를 재귀적으로 효율화하기 | paper-review | 개발자 AI 워크플로 | | arXiv 2609.20519 — unattended coding/research agent loop의 token budget, exploration cost, feedback loop를 harness 수준에서 재귀적으로 개선하는 접근 |
| 아이디어 | 미정 | Containarium — SSH-native isolation과 eBPF policy를 갖춘 agent runtime | tutorial | Agent/MCP 실전 | | GitHub repo 기반 — SSH-native control plane, eBPF egress policy, Kubernetes/LXC backend, MCP-native CLI를 tool-use agent sandbox 운영 경계로 소개 |
| 아이디어 | 미정 | [논문 리뷰] Harness Design for Coding Agents — planning·action space·context management ablation | paper-review | 개발자 AI 워크플로 | | arXiv 2609.20804 — fixed loop에서 planning, action space, context-management strategy를 분리 평가해 coding agent harness component별 성능/비용 trade-off를 분석 |
| 아이디어 | 미정 | Claw-Eval — human-verified task로 LLM agent harness를 평가하기 | tutorial | AI 평가/운영 | | GitHub repo 기반 — human-verified tasks, agent execution boundary, reproducibility를 내부 agent regression suite 설계 기준으로 소개 |
| 아이디어 | 미정 | prism-coder — coding agent 세션 메모리를 local-first로 공유하기 | tutorial | 개발자 AI 워크플로 | | GitHub repo 기반 — on-device inference, MCP server, SQLite/vector search, drift detection으로 Claude Code/Cursor/Codex 간 session continuity를 만드는 방식 분석 |
| 아이디어 | 미정 | [논문 리뷰] ScienceIDE — 과학 코드베이스를 agent 학습 환경으로 바꾸기 | paper-review | 개발자 AI 워크플로 | | arXiv 2609.19134 — scientific repository를 executable environment, task generation, acceptance criteria, verifier로 바꾸는 연구 자동화/coding agent 학습 하네스 설계 |
| 아이디어 | 미정 | [논문 리뷰] Cognitive Extensions for Dual-Process Language Agents — memory와 self-reflection을 feature flag로 붙이기 | paper-review | Agent/MCP 실전 | | arXiv 2609.19128 — adaptive memory module, trigger-driven retrieval, bounded self-reflection을 ablation 가능한 agent runtime extension으로 분석 |
| 아이디어 | 미정 | [논문 리뷰] ASLEval — tool-using agent 세션의 privacy exposure displacement 측정하기 | paper-review | AI 평가/운영 | | arXiv 2609.18864 — final answer/action proxy를 넘어 visible exits 전체에서 unauthorized exposure를 측정하는 authorization-aware session-level privacy eval |
| 아이디어 | 미정 | [논문 리뷰] DualViewEval — outcome과 process signal로 agent benchmark를 압축하기 | paper-review | AI 평가/운영 | | arXiv 2609.18909 — final score와 trajectory process signal을 함께 보존하는 miniset selection, CI용 agent benchmark 비용 절감 설계 |
| 아이디어 | 미정 | Mastra — TypeScript agent framework를 production AI app 뼈대로 보기 | tutorial | Agent/MCP 실전 | | GitHub repo 기반 — TypeScript-first agent/workflow/memory/eval/deploy control plane을 backend 통합 기준으로 소개 |
| 아이디어 | 미정 | [논문 리뷰] Embodied-BenchForge — embodied benchmark를 agentic workflow로 자동 생성하기 | paper-review | AI 평가/운영 | | arXiv 2609.13082 — user request→task artifact→artifact-specific verifier→final benchmark로 이어지는 closed-loop benchmark factory와 결함 전파 방지 설계 |
| 아이디어 | 미정 | [논문 리뷰] Agentic Societies Need a Social Harness — trust boundary를 넘는 agent coordination | paper-review | Agent/MCP 실전 | | arXiv 2609.17527 — 서로 다른 principal·권한·목표를 가진 agent society에서 social harness, audit, 상호 검증, coordination failure를 분석 |
| 아이디어 | 미정 | [논문 리뷰] JustFit — 24GB 노트북에서 200K-token LLM serving하기 | paper-review | AI 추론 최적화 | | arXiv 2609.17475 — KVExec, compressed KV execution, phase swapping으로 local long-context agent serving의 state management trade-off 정리 |
| 아이디어 | 미정 | Caura — multi-agent fleet을 위한 governed shared memory | tutorial | Agent/MCP 실전 | | GitHub repo 기반 — MCP-native shared memory, tenant/trust tier, keystone policy, audit trail, knowledge graph를 production agent memory 설계로 소개 |
| 아이디어 | 미정 | [논문 리뷰] Chain-of-Self-Questioning — LLM이 언제 답변을 보류해야 하나 | paper-review | AI 평가/운영 | | arXiv 2609.17516 — self-questioning 기반 abstention policy, selective risk control, RAG/agent 답변 commitment gate 설계 |
| 아이디어 | 미정 | [논문 리뷰] SAS — attention sparsification을 context ranking 최적화 문제로 보기 | paper-review | AI 추론 최적화 | | arXiv 2609.13141 — trainable selector, Top-K gradient blocking, dense attention distillation 한계, long-context serving latency/quality QA |
| 아이디어 | 미정 | [논문 리뷰] Tasks over Application Manuals — 장문 매뉴얼 기반 procedural reasoning 평가하기 | paper-review | 개발자 AI 워크플로 | | arXiv 2609.13005 — 수백 페이지 application manual의 조건·예외·절차 의존성을 따라가는 enterprise agent RAG verifier 설계 |
| 아이디어 | 미정 | Lobu — 조직 agent를 위한 event-sourced shared context layer | tutorial | Agent/MCP 실전 | | GitHub repo 기반 — permission-aware event history, entity graph, MCP specialist handoff를 조직 단위 agent memory/control plane으로 분석 |
| 아이디어 | 미정 | Agent-Safe Pipeline — proposal과 authorization을 분리하는 execution authority architecture | tutorial | Agent/MCP 실전 | | GitHub repo 기반 — captured intent, policy verdict, human approval, single-use execution grant, decision dossier를 mutation-safe agent 실행 경계로 정리 |
| 아이디어 | 미정 | [논문 리뷰] T1 — terminal agent RL로 long-horizon coding task를 학습시키기 | paper-review | 개발자 AI 워크플로 | | arXiv 2609.11042 — terminal environment, verifier, rollout cost, CI regression을 묶어 coding/scientific discovery agent post-training recipe로 정리 |
| 아이디어 | 미정 | [논문 리뷰] AgentGrad — multi-agent prompt를 intervention-guided로 최적화하기 | paper-review | AI 평가/운영 | | arXiv 2609.08572 — 실패 intervention point를 찾아 prompt patch와 ablation/eval harness로 반영하는 multi-agent prompt optimization 운영 루프 |
| 아이디어 | 미정 | [논문 리뷰] SWE-Bench Pro Verified — software engineering agent benchmark를 다시 검증하기 | paper-review | 개발자 AI 워크플로 | | arXiv 2609.08149 — issue validity, flaky/weak test 제거, verifier 신뢰도, 내부 coding-agent eval dataset QA 절차 |
| 아이디어 | 미정 | remnic — user-aware agent를 위한 scoped memory와 provenance layer | tutorial | 개발자 AI 워크플로 | | GitHub repo 기반 — scoped memory, provenance, retrieval quality, correction loop, MCP/HTTP access를 personal/enterprise agent memory subsystem으로 분석 |
| 발행완료 | 2026-09-15 | [논문 리뷰] EvoSafeHarness — agent safety harness를 model·domain별로 진화시키기 | paper-review | AI 평가/운영 | | `content/posts/2026-09-15-evosafeharness-model-domain-agent-safety-harness.mdx` — 자동 발행 완료, arXiv 2609.05903 — indirect prompt injection과 harmful request를 model/domain-specific enforcement layer로 막는 안전 하네스 최적화, over-blocking/under-blocking trade-off 분석 |
| 발행완료 | 2026-09-17 | [논문 리뷰] Corrupt Plans, Clean Traces — CoT monitor를 우회하는 plan injection | paper-review | AI 평가/운영 | | `content/posts/2026-09-17-corrupt-plans-clean-traces-plan-injection.mdx` — 자동 발행 완료, arXiv 2609.15989 — chain-of-thought monitor를 안전 gate로 쓸 때 hidden plan injection과 clean trace evasion이 만드는 한계, action-level audit와 counterfactual 검증 필요성 분석 |
| 아이디어 | 미정 | [논문 리뷰] State-Conditioned Evidence — coding agent에게 “이미 읽은 것”을 빼고 충분한 근거 찾기 | paper-review | 개발자 AI 워크플로 | | arXiv 2609.20050 — 현재 agent state를 보완하는 minimal sufficient evidence recovery로 repo-scale coding agent retrieval을 top-k relevance가 아닌 state complement 문제로 분석 |
| 아이디어 | 미정 | [논문 리뷰] JustMem — 장기 대화 메모리에서 just-enough retrieval 하기 | paper-review | 개발자 AI 워크플로 | | arXiv 2609.19877 — multi-session memory access mode, sufficiency/compactness, privacy exposure를 개인화 agent memory 운영 체크리스트로 정리 |
| 아이디어 | 미정 | [논문 리뷰] PrefixBench-H100 — prefix reuse가 LLM serving TTFT를 얼마나 줄이나 | paper-review | AI 추론 최적화 | | arXiv 2609.19657 — 반복 system prompt/RAG template/agent history에서 KV-cache prefix reuse, TTFT, cache hit 조건을 agent runtime serving contract 관점으로 분석 |
| 아이디어 | 미정 | AgentCompass — LLM/VLM agent capability를 체계적으로 평가하는 인프라 | tutorial | AI 평가/운영 | | GitHub repo 기반 — capability taxonomy, task adapter, execution log, evaluator plugin을 사내 agent regression suite 설계 기준으로 소개 |
| 아이디어 | 미정 | ASSERT — 요구사항 기반으로 agent behavior test를 생성하는 eval harness | tutorial | AI 평가/운영 | | GitHub repo 기반 — requirement-to-test pipeline, OTel trace, local-first artifact, CI gate와 failure dossier를 production LLM app QA로 정리 |
| 아이디어 | 미정 | [논문 리뷰] The Router Within — frozen LLM 내부의 skill routing 능력 끌어내기 | paper-review | Agent/MCP 실전 | | arXiv 2609.15982 — skill metadata stuffing과 retriever 기반 routing의 한계를 넘어 native routing signal, skill library scaling, tool/MCP registry 선택 정책 정리 |
| 아이디어 | 미정 | [논문 리뷰] Vulnerability Localization Benchmark — repo-scale 보안 분석 agent의 위치 찾기 능력 측정 | paper-review | 개발자 AI 워크플로 | | arXiv 2609.15939 — 보안 agent가 취약점 설명에서 relevant code localization으로 넘어가기 위한 repository-scale search, symbol/context selection, triage-to-fix pipeline 평가 |
| 아이디어 | 미정 | deja-vu — 여러 coding agent가 공유하는 session-history 기반 memory layer | tutorial | 개발자 AI 워크플로 | | GitHub repo 기반 — Claude Code/Codex/Cursor/Copilot CLI 사이에서 session history를 공용 operational memory로 재사용하는 cross-agent memory workflow 분석 |
| 아이디어 | 미정 | [논문 리뷰] MetroLLM-Bench — LLM을 transit kiosk policy layer로 평가하기 | paper-review | AI 평가/운영 | | arXiv 2609.10016 — structured tool call과 machine-renderable terminal state를 요구하는 955-case transit kiosk benchmark, deterministic scorer와 semantic judge tier를 enterprise agent eval로 정리 |
| 아이디어 | 미정 | [논문 리뷰] HyQuant — LLM attention에서 중요한 state만 고정밀로 남기기 | paper-review | AI 추론 최적화 | | arXiv 2608.27875 — vertical-line token과 local-window state를 고정밀로 유지하는 hybrid attention quantization, serving latency/accuracy QA 체크리스트 |
| 아이디어 | 미정 | coder_eval — coding agent용 Playwright처럼 쓰는 eval harness | tutorial | 개발자 AI 워크플로 | | GitHub repo 기반 — Claude Code/Codex/Gemini/OpenCode를 팀 자체 task에서 sandbox YAML suite, activation check, A/B experiment, CI gate로 평가하는 방식 소개 |
| 아이디어 | 미정 | coddy-agent — static Go binary 하나로 묶은 general-purpose agent runtime | tutorial | Agent/MCP 실전 | | GitHub repo 기반 — TUI, ACP server, OpenAI-compatible API, cron, swarm relay, rules/skills/subagents/hooks, MCP, memory/compaction을 deployable runtime 관점으로 분석 |
| 아이디어 | 미정 | [논문 리뷰] Memory as Plans — long-horizon agent memory를 계획 기록으로 쓰기 | paper-review | 개발자 AI 워크플로 | | HF/arXiv 2609.11561 — memory-grounded planning, plan-conditioned execution, long-term multimodal episodic context를 embodied/browser agent runtime 관점으로 정리 |
| 아이디어 | 미정 | [논문 리뷰] Negative Self-Distillation — flawed reasoning에서 멀어지게 학습하기 | paper-review | AI 평가/운영 | | HF/arXiv 2609.11699 — OPSD의 uncertainty suppression 실패 모드, negative objective, agent reflection/retry policy 학습 신호 분석 |
| 아이디어 | 미정 | [논문 리뷰] Beyond Solver Verdicts — autoformalization reward를 verdict 너머로 만들기 | paper-review | AI 평가/운영 | | HF/arXiv 2609.11085 — verdict-preserving unfaithfulness, reference-equivalence, generative verification을 formal reasoning agent QA gate로 정리 |
| 아이디어 | 미정 | trpc-agent-go — Go backend 팀을 위한 production agent framework | tutorial | Agent/MCP 실전 | | GitHub repo 기반 — graph workflow, tool/memory, A2A/AG-UI/MCP, evaluation/observability를 Go production agent checklist로 소개 |
| 아이디어 | 미정 | Kiln — eval·RAG·agent optimization을 한 작업대에서 묶기 | tutorial | AI 평가/운영 | | GitHub repo 기반 — dataset management, eval harness, synthetic data, MCP integration, optimization workflow를 AI 시스템 개선 루프로 분석 |
| 아이디어 | 미정 | [논문 리뷰] AgentAudit — AI agent 신뢰성을 full-lifecycle로 평가하기 | paper-review | AI 평가/운영 | | arXiv 2609.09875 — capability/risk taxonomy, tool trace, audit log, regression gate를 agent 설계-실행-감사 lifecycle contract로 정리 |
| 아이디어 | 미정 | [논문 리뷰] KVShareArena — context와 checkpoint를 넘나드는 KV-cache reuse 평가하기 | paper-review | AI 추론 최적화 | | arXiv 2609.10266 — prefix/state fingerprint, checkpoint compatibility, latency/accuracy trade-off, agent runtime cache policy 분석 |
| 아이디어 | 미정 | [논문 리뷰] What Eviction Destroys — agent memory forgetting을 counterfactual로 감사하기 | paper-review | 개발자 AI 워크플로 | | arXiv 2609.08279 — restore-counterfactual audit, forgetting taxonomy, eviction policy regression을 memory 운영 체크리스트로 정리 |
| 발행완료 | 2026-09-12 | Solace Agent Mesh — event-driven multi-agent 시스템의 control plane 설계 | tool | Agent/MCP 실전 | | `content/posts/2026-09-12-solace-agent-mesh-event-driven-multi-agent-control-plane.mdx` — 자동 발행 완료, GitHub repo 기반, event bus, capability routing, async workflow, telemetry/backpressure를 production multi-agent orchestration 관점으로 소개 |
| 아이디어 | 미정 | [논문 리뷰] IBIB — model name이 아니라 serving route 단위로 enterprise AI를 측정하기 | paper-review | AI 평가/운영 | | arXiv 2609.10494 — route fingerprint, harness/version pinning, output contract를 실제 serving contract 기준의 enterprise AI regression report로 정리 |
| 아이디어 | 미정 | [논문 리뷰] Fortunate Recall — ontology-driven memory lifecycle로 persistent coherence 유지하기 | paper-review | 개발자 AI 워크플로 | | arXiv 2609.10413 — memory class별 persistence/replacement/decay policy, coherence metric, privacy-aware cleanup을 personal agent memory 운영 관점으로 분석 |
| 아이디어 | 미정 | [논문 리뷰] JarvisGUI — cross-device GUI agent의 동적 task composition | paper-review | AI 평가/운영 | | arXiv 2609.10451 — PC·모바일·웹앱을 넘나드는 GUI task에서 shared state, device handoff, local/global recovery boundary를 benchmark 관점으로 정리 |
| 아이디어 | 미정 | hud-python — RL environment와 eval을 한 번 정의해 agent 학습까지 연결하기 | tutorial | AI 평가/운영 | | GitHub repo 기반 — task definition, rollout log, reward/metric separation, train/eval 재사용 패턴을 agent improvement loop로 소개 |
| 아이디어 | 미정 | [논문 리뷰] RuleMem — conversational agent의 장기 기억을 규칙으로 관리하기 | paper-review | 개발자 AI 워크플로 | | arXiv 2609.03915 — active rule memory, 사용자 선호/제약 갱신 정책, conflict handling, memory regression test를 personal agent memory 운영 관점으로 정리 |
| 아이디어 | 미정 | [논문 리뷰] When Users Don't Ask — context-driven memory retrieval 평가하기 | paper-review | AI 평가/운영 | | arXiv 2609.03467 — explicit query 없이 필요한 기억을 꺼내는 proactive retrieval, timing/suppression/privacy guard를 memory eval checklist로 분석 |
| 아이디어 | 미정 | [논문 리뷰] Two-Stage RL for Code LLMs — 테스트 생성 agent를 sound하게 학습시키기 | paper-review | 개발자 AI 워크플로 | | arXiv 2609.03955 — soundness gate와 adversarial objective를 분리한 test generation RL, generated test validation과 PR regression workflow 설계 |
| 아이디어 | 미정 | trace-mcp — 코드 리뷰 agent를 위한 framework-aware MCP code intelligence | tutorial | Agent/MCP 실전 | | GitHub repo 기반 — symbol graph, PR review context selection, local-only privacy, token budget 절감을 coding agent context layer로 소개 |
| 아이디어 | 미정 | [논문 리뷰] Emergent Cheating and Whistleblowing — autonomous research swarm의 집단 실패 모드 | paper-review | AI 평가/운영 | | arXiv 2609.04170 — shared tool infrastructure, cheating/sabotage/whistleblowing, research agent audit trail과 governance 설계 |
| 아이디어 | 미정 | [논문 리뷰] Natural Language Interaction Protocol — 이기종 agent를 자연어 contract로 연결하기 | paper-review | Agent/MCP 실전 | | arXiv 2609.04135 — heterogeneous framework 간 intent/capability/failure contract, tool handoff, auditability를 agent protocol 관점으로 정리 |
| 아이디어 | 미정 | [논문 리뷰] DRACO — dynamic rubric으로 long-horizon agent credit assignment 하기 | paper-review | AI 평가/운영 | | arXiv 2609.04094 — verifiable reward 없는 agent task에서 dynamic rubric, fine-grained credit, regression signal 설계 |
| 아이디어 | 미정 | open-multi-agent — approval gate와 verifiable record를 기본값으로 둔 multi-agent framework | tutorial | Agent/MCP 실전 | | GitHub repo 기반 — TypeScript runtime, consequential action approval, run record, self-hosted multi-agent 운영 체크리스트 |
| 아이디어 | 미정 | Waku Agent — loop·memory·eval을 코드로 소유하는 local-first agent harness | tutorial | 개발자 AI 워크플로 | | GitHub repo 기반 — loop ownership, memory structure, eval hook, local-first harness 구조를 readable codebase 관점으로 분석 |
| 아이디어 | 미정 | [논문 리뷰] SWE-Gate — 코딩 에이전트 평가는 테스트 통과만으로 충분한가 | paper-review | 개발자 AI 워크플로 | | arXiv 2609.04167 — functional test 통과 이후 review-quality rubric, hidden failure, CI verifier와 human review boundary 설계 |
| 아이디어 | 미정 | [논문 리뷰] Terminal-Universe — agent trajectory를 실행 가능한 terminal environment로 바꾸기 | paper-review | 개발자 AI 워크플로 | | arXiv 2609.04148 — trajectory-to-environment 변환, sandbox 재현성, verifier, terminal agent post-training 데이터 flywheel |
| 아이디어 | 미정 | [논문 리뷰] Clean Engineering, Unstable Measurement — LLM judge endpoint는 얼마나 안정적인가 | paper-review | AI 평가/운영 | | arXiv 2609.04198 — shared endpoint variance, judge pinning, drift monitor를 agent regression gate 운영 리스크로 정리 |
| 아이디어 | 미정 | golf-mcp — production MCP server framework를 고를 때 봐야 할 것 | tutorial | Agent/MCP 실전 | | GitHub repo 기반 — auth, observability, debugger, telemetry, runtime 배포를 MCP 운영면 체크리스트로 분석 |
| 아이디어 | 미정 | raindrop-ai/workshop — coding agent에게 eval 작성 권한을 주는 방식 | tutorial | 개발자 AI 워크플로 | | GitHub repo 기반 — agent-written eval, sandbox 실행, regression dataset, PR review loop를 coding agent harness로 정리 |
| 아이디어 | 미정 | [논문 리뷰] Hidden User Models — 개인화 agent 메모리가 무엇을 누설할 수 있나 | paper-review | 논문 리뷰 | | arXiv 2609.03815 — memory에서 압축된 user model의 behavior leakage, provenance/삭제 정책, 개인화 agent privacy boundary |
| 아이디어 | 미정 | [논문 리뷰] KC-Bench — LLM agent가 충돌하는 지식을 어떻게 조정하나 | paper-review | AI 평가/운영 | | arXiv 2609.03588 — 사용자 지시·모델 지식·tool observation 충돌 처리, stale fact 우선순위, multi-turn benchmark |
| 아이디어 | 미정 | [논문 리뷰] Speculative Macro Commit — tool-using agent의 serial latency 줄이기 | paper-review | 개발자 AI 워크플로 | | arXiv 2609.03236 — speculative macro action, rollback boundary, verification checkpoint로 tool agent 왕복 지연 줄이기 |
| 아이디어 | 미정 | Strands harness-sdk: production agent harness를 직접 통제하는 SDK | tutorial | Agent/MCP 실전 | | GitHub repo 기반 — loop ownership, tool boundary, telemetry/eval hook, provider portability를 production agent harness 관점으로 정리 |
...
| 발행완료 | 2026-07-25 | [논문 리뷰] Agentic Context Management — agent memory를 lifecycle로 다루기 | paper-review | 논문 리뷰 | 17 | `content/posts/2026-07-25-agentic-context-management-lifecycle-architecture.mdx` — 자동 발행 완료, arXiv 2607.21503, context lifecycle, compaction trigger, memory tier, cost ceiling을 agent runtime 설계로 정리 |
...
| 아이디어 | 미정 | [논문 리뷰] TurnSight — tool-integrated reasoning의 turn-level hindsight distillation | paper-review | 논문 리뷰 | | arXiv 2608.04007 — tool call별 credit assignment, self-distillation, 실패 turn 재학습을 agent RL/eval pipeline으로 정리 |
| 아이디어 | 미정 | [논문 리뷰] PAST-Bench — personal agent의 recursive self-improvement 평가 | paper-review | AI 평가/운영 | | arXiv 2608.04003 — preferences·task history·tool routine·memory가 다음 행동 개선으로 이어지는지 보는 personal agent benchmark |
...
| 발행완료 | 2026-08-08 | [논문 리뷰] TRAJDEBUG — long-horizon agent trajectory에서 첫 치명 오류 찾기 | paper-review | AI 평가/운영 | | `content/posts/2026-08-08-trajdebug-critical-failure-localization.mdx` — 자동 발행 완료, arXiv 2608.06346, critical failure localization, replay, regression test를 agent observability 운영 패턴으로 정리 |
...
| 발행완료 | 2026-08-11 | [논문 리뷰] TEPA — stale memory를 철회하는 conflict-robust language agent | paper-review | 논문 리뷰 | | `content/posts/2026-08-11-tepa-revoking-stale-agent-memories.mdx` — 자동 발행 완료, arXiv 2608.07429, stale memory detection, conflict handling, temporal preference update, memory pollution regression test 설계 |
...
| 발행완료 | 2026-08-18 | [논문 리뷰] Handover of In-Context Learning State — 세션 경계를 넘는 agent state 전달 | paper-review | 논문 리뷰 | | `content/posts/2026-08-18-handover-in-context-learning-state.mdx` — 자동 발행 완료, arXiv 2608.14528, session handover를 task-relative ICL state 전달과 resume packet 설계 문제로 정리 |
...
| 발행완료 | 2026-08-20 | [논문 리뷰] StagedWorkspace — knowledge-work agent에 versioned workspace가 필요한 이유 | paper-review | 개발자 AI 워크플로 | | `content/posts/2026-08-20-stagedworkspace-versioned-workspace-agents.mdx` — 자동 발행 완료, arXiv 2608.18050, parsed view/native file/diff/submission artifact를 workspace-state contract로 묶는 agent harness 설계 |
...
| 아이디어 | 미정 | [논문 리뷰] SPADE — adaptive executable environment에서 self-play agent RL 하기 | paper-review | AI 평가/운영 | | arXiv 2608.19197 — self-play goal generation, executable environment, verifier drift를 agentic RL harness 설계로 정리 |
| 아이디어 | 미정 | [논문 리뷰] Harness Continual Learning — model parameter 밖의 agent 상태 업데이트 | paper-review | Agent/MCP 실전 | | arXiv 2608.19013 — prompt/memory/tool/skill/routing rule 변경과 regression/rollback gate를 agent 운영법으로 해석 |
| 아이디어 | 미정 | [논문 리뷰] MissDiag — incomplete KG-RAG에서 실패 원인을 진단하기 | paper-review | Agentic RAG | | arXiv 2608.18489 — sparse/outdated KG 환경에서 missing evidence diagnosis와 GraphRAG QA 설계 |
| 아이디어 | 미정 | SkillSpector — Claude Code·Codex·MCP skill supply chain을 스캔하기 | tutorial | 개발자 AI 워크플로 | | GitHub repo 기반 — agent skill 설치 전 prompt injection, exfiltration, malicious hook을 검사하는 security gate |
| 발행완료 | 2026-08-22 | [논문 리뷰] MidTool — agentic tool-use를 mid-training 데이터 합성으로 키우기 | paper-review | Agent/MCP 실전 | | `content/posts/2026-08-22-midtool-agentic-tool-use-mid-training.mdx` — 자동 발행 완료, arXiv 2608.20314, tool task 생성, trajectory 품질, verifier, generalization failure를 agent post-training 운영법으로 정리 |
| 아이디어 | 미정 | [논문 리뷰] Task-CoEvolve — harness optimization의 검증 비용 줄이기 | paper-review | 개발자 AI 워크플로 | | arXiv 2608.20169 — adaptive validation task selection, regression risk, rollback gate를 harness CI 비용 최적화 관점으로 해석 |
...
| 발행완료 | 2026-08-25 | [논문 리뷰] Utility Under Attack — agent memory poisoning이 실제 효용을 얼마나 깎는가 | paper-review | AI 평가/운영 | | `content/posts/2026-08-25-utility-under-attack-memory-poisoning.mdx` — 자동 발행 완료, arXiv 2608.21230, false assertion poisoning, provenance ranking 한계, memory quarantine·repair workflow를 persistent memory agent 보안 관점으로 정리 |
...
| 발행완료 | 2026-08-27 | [논문 리뷰] Recuris — long-horizon agent memory를 재귀적으로 진화시키기 | paper-review | 논문 리뷰 | 19 | `content/posts/2026-08-27-recuris-recursive-memory-evolution-agent-harness.mdx` — 자동 발행 완료, arXiv 2608.24876, working/experiential/skill memory 분리, failure localization, validation-gated skill update를 agent harness 운영법으로 정리 |
...
| 발행완료 | 2026-08-29 | rulesync — 여러 coding agent 규칙 파일을 한 소스에서 동기화하기 | tool | 개발자 AI 워크플로 | | `content/posts/2026-08-29-rulesync-agent-rules-configuration-drift.mdx` — 자동 발행 완료, GitHub repo 기반, canonical rule source, provider-specific export, AGENTS.md/Cursor/Codex 설정 drift 방지 |
...
| 아이디어 | 미정 | [논문 리뷰] Aspire — vague goal에서 self-evolving agent를 만드는 법 | paper-review | AI 평가/운영 | | arXiv 2608.31111 — 목표 해석, capability gap 진단, 학습 과제 생성, 개선 검증을 self-improvement harness로 정리 |
| 아이디어 | 미정 | [논문 리뷰] S3Gym — self-testing과 self-judging으로 agent를 개선할 수 있나 | paper-review | AI 평가/운영 | | arXiv 2608.31100 — self-test generation, self-judge calibration, improvement signal, regression guard |
| 아이디어 | 미정 | [논문 리뷰] Adaptive Structuring — 비정형 데이터를 token-efficient agent substrate로 바꾸기 | paper-review | Agentic RAG | | arXiv 2608.31082 — 비정형 문서를 evidence table/structured substrate로 바꿔 token cost와 tool dispatch 비용 줄이기 |
| 아이디어 | 미정 | Gram: MCP·Skills·agent gateway를 조직 단위로 보안·배포하는 스택 | tutorial | Agent/MCP 실전 | | GitHub repo 기반 — API/MCP 연결, policy, observability, skill distribution boundary를 platform architecture로 소개 |
