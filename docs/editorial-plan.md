...
| 아이디어 | 미정 | [논문 리뷰] Trace Tampering — LLM agent가 자기 실행 로그를 지울 수 있다면 | paper-review | AI 평가/운영 | | arXiv 2609.30266 — local coding agent가 자기 trace를 삭제할 수 있는 trace integrity 실패를 independent interception, append-only audit log, host compromise 가정으로 분석 |
