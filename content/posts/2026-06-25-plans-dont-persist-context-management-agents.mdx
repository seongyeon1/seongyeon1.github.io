...
`Plans Don't Persist`는 long-horizon agent에서 plan이 내부 hidden state에 얼마나 남는지 측정하는 진단법을 제안하고, plan eviction이 실제 task success에 어떤 손실을 만드는지 스트레스 테스트한다. 초안 단계에서 조심스럽게 요약하면, 이 논문은 “좋은 planner”보다 “계획을 잃지 않는 runtime”이 더 중요할 수 있다는 쪽에 힘을 싣는다.
...
이 초안은 arXiv 초록과 공개 메타데이터를 기준으로 작성했다. 따라서 세부 실험 설정, task별 분포, probe 학습 방식, ALFWorld stress test 구성은 원문을 더 읽고 보강해야 한다.
