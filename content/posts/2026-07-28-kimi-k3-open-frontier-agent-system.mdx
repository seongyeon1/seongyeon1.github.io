...
이 리포트는 용어가 꽤 많다. 뒤의 내용을 읽기 전에 아래 정도만 잡고 가면 훨씬 편하다.

| 용어 | 쉽게 말하면 | 왜 중요한가 |
|---|---|---|
| MoE, Mixture-of-Experts | 모델 안에 “전문가 모듈”을 많이 두고, 입력 token마다 일부 전문가만 호출하는 구조 | 전체 모델은 매우 크지만 매 token마다 일부만 쓰기 때문에 성능과 비용을 같이 노릴 수 있다. |
| Total parameters | 모델 전체에 들어 있는 모든 가중치 수 | Kimi K3는 2.78T로 매우 크다. 다만 매번 전부 쓰지는 않는다. |
| Activated parameters | 한 token을 처리할 때 실제로 켜지는 가중치 수 | Kimi K3는 104.2B만 활성화한다. MoE의 비용 효율을 보여주는 핵심 숫자다. |
| Expert | MoE 안의 개별 전문가 네트워크 | Kimi K3는 896개 routed expert 중 token마다 16개를 고른다. |
| Routed expert / Shared expert | 입력에 따라 선택되는 전문가 / 항상 같이 쓰이는 공통 전문가 | 전문화와 공통 능력 사이의 균형을 잡기 위한 장치다. |
| KDA, Kimi Delta Attention | 긴 문맥을 고정 크기 상태로 압축해 넘기는 attention 계열 | 1M token context를 full attention 비용으로 처리하기 어렵기 때문에 필요하다. |
| MLA, Multi-head Latent Attention | key-value 정보를 latent vector로 압축해 attention 비용을 줄이는 방식 | KDA만 쓰면 약해질 수 있는 전역 token interaction을 보완한다. |
| Hybrid KDA-MLA | KDA layer를 주로 쓰고, 중간중간 MLA layer로 전역 상호작용을 넣는 구조 | “대부분은 싸게, 가끔은 넓게 본다”는 long-context 절충안이다. |
| NoPE | RoPE 같은 명시적 positional embedding을 쓰지 않는 선택 | 1M context로 늘릴 때 position encoding 재조정 부담을 줄이려는 방향이다. |
| Attention Residuals | token 방향이 아니라 layer 깊이 방향에서도 필요한 이전 표현을 골라 보는 구조 | 깊은 모델에서 정보가 중간에 희석되는 문제를 줄이려는 장치다. |
| Stable LatentMoE | expert를 많이 쓰면서도 activation 폭주와 load imbalance를 줄이는 MoE 설계 | 896개 expert를 안정적으로 학습·서빙하려면 구조적 안정화가 필요하다. |
| Quantile Balancing | expert별 부하가 너무 쏠리지 않게 routing bias를 조정하는 방식 | 일부 GPU나 expert만 과부하되는 MoE 병목을 줄인다. |
| SFT | 사람이 선호하는 답변·작업 예시로 supervised fine-tuning하는 단계 | RL 전에 기본 agent 행동을 잡는 워밍업에 가깝다. |
| RL | 보상 신호로 모델 행동을 개선하는 post-training | 단순 답변이 아니라 도구 사용, 코딩, 장기 작업 성공률을 끌어올리는 데 쓰인다. |
| On-policy distillation | 현재 모델이 실제로 만든 trajectory를 기반으로 여러 teacher 정책을 하나로 합치는 방식 | low/high/max reasoning effort별 능력을 하나의 모델에 통합하는 데 사용된다. |
| Agent harness | 모델이 도구, 파일, 브라우저, 메모리, subagent 등을 쓰는 실행 껍데기 | 같은 모델도 harness가 다르면 agent 성능이 크게 달라진다. |
| Prefix cache / KV cache | 이미 처리한 긴 앞부분의 계산 결과를 재사용하는 캐시 | 400K token prefix를 매번 다시 읽으면 비용이 터지기 때문에 agent serving의 핵심이다. |
| Sandbox / microVM | agent가 코드를 실행하거나 웹 작업을 할 수 있는 격리 실행환경 | 장기 agentic RL에서는 수천만 개의 안전하고 빠른 실행환경이 필요하다. |

한 문장으로 줄이면, Kimi K3의 용어들은 대부분 **“엄청 큰 오픈 모델을 긴 문맥·도구 사용·비전·코딩 작업에 실제 비용 안에서 굴리려면 무엇이 필요한가”**에 대한 답이다.
